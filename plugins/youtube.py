...
async def ytdl(_, message):
    ...
    try:
        if userLastDownloadTime > datetime.now():
            ...
            await message.reply_text(f"`Tunggu {wait_time} Menit untuk melakukan URL yang lain`")
            return
    except:
        pass
    ...
    try:
        ...
        await message.reply_text("`Gagal mengambil URL Youtube😔 \nCoba Gunakan URL Yang baru`")
        return
    buttons = InlineKeyboardMarkup(list(create_buttons(formats)))
    sentm = await message.reply_text("Sedang Mencari URL 🔎")
    try:
